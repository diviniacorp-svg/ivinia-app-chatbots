...
import {
  ...
  Phone, Mail, MapPin, Instagram, Copy,
} from 'lucide-react'
...
function ProductCard({ pid, client }: { pid: string; client: Client }) {
  ...
  if (pid === 'nucleus' || pid === 'web') {
    const website = cfg.website || cfg.nucleus_url
    if (website) links.push({ label: 'Visitar sitio', href: website, icon: <Globe size={13} />, external: true })
    const panelUrl = cfg.nucleus_panel_url
    if (panelUrl) links.push({ label: 'Panel interno', href: panelUrl, icon: <Cpu size={13} />, external: true })
  }
  if ((pid === 'chatbot' || pid === 'nucleus') && client.chatbot_id) {
  ...
  if (pid === 'content') {
    links.push({ label: 'Gestionar contenido', href: `/content?client=${client.id}`, icon: <Sparkles size={13} /> })
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {links.map(link => (
            ...
            >
              ...
              <ExternalLink size={11} style={{ color: 'var(--muted)', opacity: 0.6 }} />
            </a>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      {editing ? (
        <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
          {multiline
            ? <textarea style={{ ...inp, minHeight: 60, resize: 'vertical' }} value={val} onChange={e => setVal(e.target.value)} />
            : <input style={inp} type={type} value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && save()} />
          }
        ...
        >
          {val || <span style={{ fontStyle: 'italic', color: 'var(--muted)' }}>Agregar...</span>}
          <Pencil size={10} style={{ color: 'var(--muted)', opacity: 0.5, marginLeft: 'auto' }} />
        </div>
      )}
    </div>
  )
    ...
    <div style={{ background: 'var(--paper-2)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--paper)', borderBottom: '1px solid var(--line)', padding: '20px 32px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
          {/* Avatar */}
          <div style={{
            width: 56, height: 56, borderRadius: 14, background: color, flexShrink: 0,
          ...
          {Number(client.mrr) > 0 && (
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontFamily: 'var(--f-display)', fontStyle: 'italic', fontWeight: 700, fontSize: 28, color: 'var(--ink)', letterSpacing: '-0.03em' }}>
                {new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 0 }).format(Number(client.mrr))}
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ padding: '28px 32px', display: 'grid', gap: 24, gridTemplateColumns: '1fr 340px', maxWidth: 1200 }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 16, border: '1px solid var(--line)', padding: '24px' }}>
            ...
            {products.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--muted)', fontStyle: 'italic' }}>Sin productos registrados — editá desde el lápiz</p>
            ) : (
              <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))' }}>
                {products.map(pid => <ProductCard key={pid} pid={pid} client={client} />)}
              </div>
            )}
          </div>

          {/* Website / URLs */}
          {(cfg.website || cfg.nucleus_panel_url) && (
            <div style={{ background: 'var(--paper)', borderRadius: 16, border: '1px solid var(--line)', padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {cfg.website && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--paper-2)', borderRadius: 10, border: '1px solid var(--line)' }}>
                    <Globe size={14} style={{ color: '#38BDF8', flexShrink: 0 }} />
                    <span style={{ flex: 1, fontSize: 13, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cfg.website}</span>
                    <a href={cfg.website} target="_blank" rel="noopener noreferrer"
                      style={{ color: 'var(--muted)', display: 'flex' }}>
                      <ExternalLink size={13} />
                    </a>
                    <button onClick={() => copyText(cfg.website!, setCopied)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', display: 'flex', padding: 0 }}>
                      {copied ? <Check size={13} style={{ color: '#10b981' }} /> : <Copy size={13} />}
                    </button>
                  </div>
                )}
                {cfg.nucleus_panel_url && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--paper-2)', borderRadius: 10, border: '1px solid var(--line)' }}>
                    <Cpu size={14} style={{ color: '#A78BFA', flexShrink: 0 }} />
                    <span style={{ flex: 1, fontSize: 13, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cfg.nucleus_panel_url}</span>
                    <a href={cfg.nucleus_panel_url} target="_blank" rel="noopener noreferrer"
                      style={{ color: 'var(--muted)', display: 'flex' }}>
                      <ExternalLink size={13} />
                    </a>
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 16, border: '1px solid var(--line)', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <Mail size={13} style={{ color: 'var(--muted)', marginTop: 6, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <InlineEdit label="Email" value={client.email} fieldKey="email" clientId={client.id} onSaved={v => updateField('email', v)} type="email" />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <Phone size={13} style={{ color: 'var(--muted)', marginTop: 6, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <InlineEdit label="WhatsApp / Teléfono" value={client.phone || ''} fieldKey="phone" clientId={client.id} onSaved={v => updateField('phone', v)} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <MapPin size={13} style={{ color: 'var(--muted)', marginTop: 6, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <InlineEdit label="Ciudad" value={cfg.city || ''} fieldKey="city" clientId={client.id} onSaved={v => updateField('city', v)} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <Instagram size={13} style={{ color: 'var(--muted)', marginTop: 6, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <InlineEdit label="Instagram" value={cfg.instagram || ''} fieldKey="instagram" clientId={client.id} onSaved={v => updateField('instagram', v)} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <Globe size={13} style={{ color: 'var(--muted)', marginTop: 6, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <InlineEdit label="Sitio web" value={cfg.website || ''} fieldKey="website" clientId={client.id} onSaved={v => updateField('website', v)} />
                </div>
              </div>
            </div>
          </div>
          ...
          {client.chatbot_id && (
            <div style={{ background: 'var(--paper)', borderRadius: 16, border: '1px solid var(--line)', padding: '16px 20px' }}>
              ...
              <code style={{ fontSize: 12, fontFamily: 'var(--f-mono)', color: 'var(--ink)', background: 'var(--paper-2)', padding: '4px 8px', borderRadius: 6 }}>
                {client.chatbot_id}
              </code>
            </div>
          )}
