...
// GET /api/seed/demo-rubros
// Crea demos coherentes para todos los rubros de /rubros.
// Idempotente por chatbot_id.
...
const DEMOS = [
  ...
  // ── ESTÉTICA (covers: estetica, spa, masajes, depilacion, bronceado, maquillaje) ──
  {
  ...
  // ── CLÍNICA / MÉDICO (covers: clinica, kinesiologia, fonoaudiologia, oftalmologia, podologia) ──
  {
  ...
  // ── VETERINARIA (covers: veterinaria, peluqueriacanina, hotelmascotas, adiestramiento) ──
  {
  ...
  // ── CROSSFIT (covers: crossfit, artesmarciales, personaltrainer) ─────────────
  {
  ...
  // ── TALLER MECÁNICO (covers: taller, gomeria, lavadoautos, detailing) ─────────
  {
  ...
  // ── ACADEMIA DE IDIOMAS (covers: idiomas, clases, musica, autoescuela) ────────
  {
  ...
  // ── ESTUDIO LEGAL (covers: abogado, escribano, consultoria, arquitectura, contabilidad) ─
  {
  ...
  // ── SALÓN DE FIESTAS (covers: salon, fotografia, catering) ───────────────────
  {
...
export async function GET() {
  ...
  const resultados: Array<{ nombre: string; chatbot_id: string; turneroUrl: string; error?: string }> = []
  ...
  for (const demo of DEMOS) {
    try {
      ...
      if (clientError || !client) {
        resultados.push({ nombre: demo.company_name, chatbot_id: demo.chatbot_id, turneroUrl: '', error: clientError?.message })
        continue
      }
      ...
      const turneroUrl = `${appUrl}/reservas/${demo.chatbot_id}`

      await db.from('clients').update({
        custom_config: {
          ...customConfig,
          turnero_url: turneroUrl,
          panel_url: `${appUrl}/panel/${configId}`,
          panel_pin: demo.pin,
          provisioned_at: new Date().toISOString(),
        },
      }).eq('id', clientId)

      resultados.push({ nombre: demo.company_name, chatbot_id: demo.chatbot_id, turneroUrl })
    } catch (e) {
      resultados.push({ nombre: demo.company_name, chatbot_id: demo.chatbot_id, turneroUrl: '', error: String(e) })
    }
  }
